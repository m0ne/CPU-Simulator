...
describe('CpuCycle.vue Component', () => {
  ...
  it('succeeds if NextStepButton rendered', () => {
    const wrapper = shallowMount(CpuCycle, {
      propsData: { allSteps: steps, currentStep: steps[0] },
    });
    expect(wrapper.findAllComponents(NextStepButton).length).to.be.equal(1);
  });
  it('succeeds if PreviousStepButton rendered', () => {
    const wrapper = shallowMount(CpuCycle, {
      propsData: { allSteps: steps, currentStep: steps[0] },
    });
    expect(wrapper.findAllComponents(PreviousStepButton).length).to.be.equal(1);
  });
});
