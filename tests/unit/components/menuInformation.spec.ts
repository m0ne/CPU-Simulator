...
describe('Controls.vue Component', () => {
  it('succeeds if MenuInformation with empty message rendered', () => {
    const wrapper = shallowMount(Controls, {
      propsData: { changeHistory: emptyChangeHistory },
    });
    expect(wrapper.text()).to.be.equal('Go back to code editorNothing has changed yetShow past instructions and changesAnimation SpeedTheme Switcher');
  });
  it('succeeds if Controls with changeHistory rendered', () => {
    const wrapper = shallowMount(Controls, {
      propsData: { changeHistory },
    });
    expect(wrapper.text()).to.be.equal(`Go back to code editor${changeHistoryString} Show past instructions and changesAnimation SpeedTheme Switcher`);
  });
  it('succeeds if Controls with right number of elements rendered', () => {
    const wrapper = shallowMount(Controls, {
      propsData: { changeHistory },
    });
    expect(wrapper.element.children[1].children.length).to.be.equal(2);
  });
});
