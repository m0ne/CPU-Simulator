...
<template>
  <div class="menus">
    ...
    <div id="animationSpeedDiv">
      <q-slider
        v-model="animationSpeed" color="secondary" markers snap :min="0.5" :step="0.5" :max="4" @update:model-value="setAnimationSpeed" label label-text-color="grey" :label-value="animationSpeed + 'x'">
      </q-slider>
...
export default defineComponent({
  ...
  props: {
    changeHistory: Object as PropType<Array<ChangeHistory>>,
  },
  setup(props, { emit }) {
    ...
    const initialTheme = themes.find((obj) => obj.label === document.documentElement.className);

    const selectedTheme = ref(initialTheme ? initialTheme.value : 0);
